...
1. Clone this repository and run the bootstrap script:
   ...
   You can pass extra variables directly with `-e` / `--extra-vars`:

   ```bash
   ./bootstrap.sh -e gpu_power_limit=200 -e dockge_bind_ip=0.0.0.0
   ```

   Or pass additional `ansible-playbook` flags after `--`:
   ...
   ```bash
   ./bootstrap.sh --skip-gpu -e gpu_power_limit=150 -- --check --diff
   ```
...
You can also override variables inline during testing:

```bash
./bootstrap.sh --skip-gpu -e dockge_port=9000 -e dockge_bind_ip=0.0.0.0
```
